...
def get_date(date_string, date_format="%d-%b-%y"):
    return datetime.strptime(date_string, date_format) if date_string else False
...
class GrantSynchronizer(VisionDataSynchronizer):
    ...
    def _filter_records(self, records):
        def is_valid_record(record):
            return all(key in record for key in self.REQUIRED_KEYS)
    ...
    def _item_save(record):
        ...
        if record.get("SECONDARY_DONOR_CODE", None):
            ...
            if len(secondary_donors_codes) != len(secondary_donors_names):
                return -1
            for code, name in zip(secondary_donors_codes, secondary_donors_names, strict=False):
                (
                    secondary_donor,
                    _,
