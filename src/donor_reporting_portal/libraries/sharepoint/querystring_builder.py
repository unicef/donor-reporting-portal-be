...
class QueryStringBuilder:
    ...
    def __init__(self, filters):
        ...
        if 'search' in filters:
            self.search = filters.pop('search').split(',')
        if filters:
            self.filters = filters

    def get_search_querystring(self):
        if self.search:
            search_terms_string = ' or '.join(self.search)
            return str(f'query?querytext={search_terms_string}')
    ...
    def get_querystring(self):
        querystring = ''
        if self.search:
            querystring += self.get_search_querystring()
        if self.filters:
            querystring += self.get_filter_querystring()
        return querystring
